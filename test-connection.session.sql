...
SELECT DISTINCT (name, quantity) FROM products;
...
CREATE TABLE messages(
    ...
    author_id int,
    chat_id int,
    PRIMARY KEY (chat_id, author_id) REFERENCES user_to_chat (chat_id, user_id)
);
DROP TABLE content;
...
CREATE TABLE users_task(
    ...
    CONSTRAINT "FN_LN_NOT_EMPTY" CHECK (first_name != '' OR last_name != ''),
    birthday date NOT NULL CHECK (birthday > '1960-01-01'),
    height numeric(3,2) NOT NULL CHECK (height >1.5 AND height <2.5)
);

INSERT INTO users_task (first_name, last_name, birthday, height) VALUES 
('', 'Surname', '1999-01-01', 1.6),
('Name', '', '1999-01-01', 1.8),
('', 'LastName', '1999-01-01', 1.9); */
